        ...
        <div className="fixed inset-0 flex items-center justify-center bg-black z-[1000]">
          <div className="absolute inset-[-70px] flex justify-center items-center">
            <motion.img
              ...
              className="w-2/4 h-72 object-contain"
              initial={{ opacity: 0 }}
              animate={{ opacity: 1 }}
          ...
          <div className="absolute bottom-8 right-2 lg:right-[-80px] flex gap-3 overflow-x-auto">
            {galleryImages.slice(0, 3).map((img, index) => (
              <motion.div
                key={index}
                className={`relative w-20 lg:w-40 h-16 lg:h-20 rounded-lg cursor-pointer overflow-hidden flex-shrink-0 ${
            ...
            {galleryImages.length > 4 && (
              <div
                className="relative w-15 lg:w-40 h-16 lg:h-20 rounded-lg cursor-pointer overflow-hidden flex-shrink-0"
                onClick={() => setModalImageIndex(3)}
              >
                <img
                  src={galleryImages[3]}
                  alt="More"
